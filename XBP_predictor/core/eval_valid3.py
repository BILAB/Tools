...
def valid_summary(name,thr = 0,low = 5,up = 25,reverse = 0):
	sql = """ select idch,sns(dec_val,dist,%(thr)s,%(rev)s),spf(dec_val,dist,%(thr)s,%(rev)s),
	mcc(dec_val,dist,%(thr)s,%(rev)s),numbp(dist),nearbp(dist,5)
	...
	with sqlite3.connect(name) as con:
		...
		cur = con.cursor()
		#con.execute(view)
		for idch,v_sns,v_spf,v_mcc,nbp,nnbp in cur.execute(sql):
			yield idch,v_sns,v_spf,v_mcc,nbp,nnbp
	
def scop_summary(name,thr = 0,reverse = 0):
	...
	with sqlite3.connect(name,window) as con:
